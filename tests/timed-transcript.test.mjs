...
test("parses SRT cues while retaining labels, time ranges and text", () => {
  ...
  assert.deepEqual(document.cues[0], {
    index: 1,
    label: "1",
    startMs: 250,
    endMs: 2000,
    ...
    content: "先别划走，通勤是不是总担心闷？"
  });
  assert.equal(document.cues[1].label, "scene-a");
});
...
test("parses VTT header descriptions, NOTE blocks, settings and cue markup", () => {
  ...
  assert.equal(document.cues[0].content, "三秒钩子 & 事实核对");
  assert.equal(document.cues[1].label, "hook");
});
...
test("keeps TXT and Markdown paragraphs compatible with explicitly empty timing", () => {
  for (const name of ["brief.txt", "brief.md"]) {
    ...
    assert.deepEqual(document.cues.map(({ startMs, endMs }) => [startMs, endMs]), [[null, null], [null, null]]);
  }
});
